...
def check_openai_connection():
    """Validate the API key and model access without generating an answer."""
    try:
        models = client.models.list()
        available = {getattr(m, "id", "") for m in models.data}
        if MODEL not in available:
            logging.error("OpenAI model is not available for this API key: %s", MODEL)
            logging.error("Set OPENAI_MODEL to a model available to your project.")
            return False
        logging.info("OpenAI connection OK. Model: %s", MODEL)
        return True
    except AuthenticationError:
        logging.exception("OpenAI authentication failed. Check OPENAI_API_KEY in Render.")
    except RateLimitError:
        logging.exception("OpenAI rate limit/quota error while checking the API.")
    except APIError:
        logging.exception("OpenAI API error while checking the API connection.")
    except Exception:
        logging.exception("Unexpected error while checking OpenAI connection.")
    return False
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.exception("Unexpected evaluation error: %s", e)
        await update.message.reply_text(
            "⚠️ Texnik xatolik yuz berdi. Render Logs dagi yangi ERROR qatorini tekshiring."
        )
...
def main():
    ...
    if not check_openai_connection():
        logging.error("OpenAI tekshiruvi muvaffaqiyatsiz. Bot Telegramni qabul qiladi, ammo esse baholash ishlamasligi mumkin.")
